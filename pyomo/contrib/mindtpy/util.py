...
def customized_initialize(mod):
    ...
    '''
    print("====warmstart printing=====")
    for a in mod.n_responses:
        for b in mod.n_responses:
            if b>=a:
                if mod.cov_y[a,b].value > 0.0001:
    ...
    if det <= -5: 
        #print("warmstart determinant is too small. Use another FIM")
        new_fim = np.zeros((5,5))
        for i in range(5):
            new_fim[i,i] = 0.01 
    ...
    for a in mod.DimFIM:
        for b in mod.DimFIM:
            if a<=b:
                mod.TotalFIM[a,b].value = new_fim[a][b]

                grey_box_name = "ele_"+str(a)+"_"+str(b)
                mod.my_block.egb.inputs[grey_box_name] = new_fim[a][b]

    det =  np.linalg.det(np.asarray(new_fim))

    mod.my_block.egb.outputs["log_det"] = np.log(det)

    #print("Warmstart initialize FIM with: ", new_fim)
    #print("Warmstart logdet:", np.log(det))
